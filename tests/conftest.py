...
def reset_session_registry():
    """Limpa _SESSION_REGISTRY e o backend de sessão entre testes para evitar contaminação."""
    import app.session_state as ss
    import app.core.session.factory as sf

    ss._SESSION_REGISTRY.clear()
    sf.reset_session_backend(None)
    yield
    ss._SESSION_REGISTRY.clear()
    sf.reset_session_backend(None)
...
async def db_sessionmaker(tmp_path, monkeypatch):
    ...
    import app.services.evolution_service as evolution_service
    ...
    monkeypatch.setattr(evolution_service, "SessionLocal", sessionmaker)
    monkeypatch.setattr(mission_service, "SessionLocal", sessionmaker)
    monkeypatch.setattr(profile_service, "SessionLocal", sessionmaker)
    monkeypatch.setattr(segment_service, "SessionLocal", sessionmaker)
