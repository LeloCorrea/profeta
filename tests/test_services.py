...
async def test_verse_history_failure_does_not_retry_delivery(tmp_path, monkeypatch):
    ...
    result, _ = await jobs_module._send_verse_with_retry("u-hist-test", FakeBot(), logger, max_attempts=3)
...
async def test_verse_sent_as_text_only_when_audio_space_guard_fires(tmp_path, monkeypatch):
    ...
    result, _ = await jobs_module._send_verse_with_retry("u-disk-test", FakeBot(), logger, max_attempts=1)
...
async def test_openai_api_error_returns_fallback_with_error_event(monkeypatch, sample_verse):
    ...
    # retry fires one WARNING (attempt 1) + one ERROR (final attempt)
    assert len(error_events) == 2
    final_error = next(e for e in error_events if e["level"] == logging.ERROR)
    assert final_error["error_type"] == "RuntimeError"
...
def test_normalize_for_tts_converts_biblical_references():
    ...
    from app.content_service import tts_prepare

    assert tts_prepare("João 13:53") == "João capítulo 13 versículo 53"
    assert tts_prepare("Salmos 23:1 e Romanos 8:28") == (
        "Salmos capítulo 23 versículo 1 e Romanos capítulo 8 versículo 28"
    )
    assert tts_prepare("texto sem referência") == "texto sem referência"
