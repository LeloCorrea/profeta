...
- Resolver estado de sessão via TenantSessionStore
...
from app.core.engine.state_machine import next_action_for_state, next_state
...
def _journey_dict(journey) -> dict | None:
    """Serializa JourneyDefinition para dict ou retorna None."""
    if journey is None:
        return None
...
class JourneyEngine:
    ...
    Instanciação:
        engine = JourneyEngine()              # usa SessionLocal padrão
        engine = JourneyEngine(mock_sf)       # injeta factory para testes
    """
    ...
    async def verse_flow(self, tenant_id: str, user_id: str) -> EngineOutput:
        """Seleciona versículo aleatório, salva histórico e atualiza sessão."""
    ...
    async def continue_flow(self, tenant_id: str, user_id: str) -> EngineOutput:
        """Avança para o próximo passo na progressão VERSE→EXPLANATION→REFLECTION→PRAYER."""
        store = TenantSessionStore(tenant_id, user_id)
        ...
        # state == PRAYER → ciclo completo, novo versículo
        return await self.verse_flow(tenant_id, user_id)

    # ── Passos internos (também chamados pelo EngineFacade diretamente) ────────
    ...
    async def _prayer_step(self, tenant_id: str, user_id: str) -> EngineOutput:
        ...
        reflection_data = resolve_reflection(tenant_id, user_id)
        if reflection_data:
