...
class EngineFacade:
    ...
    async def execute(self, inp: EngineInput) -> EngineOutput:
        handlers = {
            ...
            "continue_what": self._continue_what,
        }
    ...
    async def _continue_what(self, inp: EngineInput) -> EngineOutput:
        """Retorna apenas o nome da próxima ação sem gerar conteúdo.

        Usado pelo bot adapter: o bot pergunta ao engine "o que vem a seguir?"
        e então chama o handler Telegram correspondente (que faz o render).
        Separa a decisão de estado (engine) da apresentação (bot).
        """
        from app.core.engine.context_resolver import resolve_verse
        from app.core.engine.state_machine import next_action_for_state
        from app.session_state import TenantSessionStore

        verse = await resolve_verse(inp.tenant_id, inp.user_id)
        if not verse:
            return EngineOutput(success=False, action="continue_what", error="no_verse_history")

        store = TenantSessionStore(inp.tenant_id, inp.user_id)
