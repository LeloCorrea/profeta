...
async def ensure_user_record(update: Update) -> None:
    ...
    try:
        from app.services.user_bootstrap import ensure_user_initialized
        await ensure_user_initialized(str(user.id))
    except Exception:
        logger.warning("Falha no bootstrap de engajamento | user=%s", user.id)
...
async def require_active_subscription(update: Update) -> bool:
    ...
    try:
        uid = str(user.id)
        if is_admin(uid) or await maybe_await(user_has_active_subscription(uid)):
            return True
    except Exception:
        logger.exception("Erro ao validar acesso.")
...
async def _init_services() -> None:
    """Inicializa banco de dados e serviços assíncronos do bot."""
    from app.init_db import init as init_db, validate_database_schema
    await init_db()
    missing = await validate_database_schema()
    if missing:
        logger.error(
            "missing_tables_detected | bot iniciou com tabelas faltando: %s",
            ", ".join(sorted(missing)),
        )
